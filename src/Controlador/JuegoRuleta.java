...
public class JuegoRuleta {
    ...
    private Map<Jugador, List<Apuesta>> apuestas;    private int numeroRuleta;
    public JuegoRuleta() {
    ...
    public boolean procesarApuesta(Jugador jugadorActual) {
        ...
        if(apuesta != null) {
            ...
            juego.agregarApuesta(jugadorActual,apuesta);
        }
        return true;
    ...
    public void agregarApuesta(Jugador jugador, Apuesta apuesta) {
        ...
        apuestas.get(jugador).add(apuesta); // Agrega la nueva apuesta a la lista de apuestas del jugador

    }

    public List<Apuesta> obtenerApuestas(Jugador jugador) {
        return apuestas.get(jugador);
    }

    public List<Jugador> verificarApuestas(int resultadoRuleta) {
        List<Jugador> ganadores = new ArrayList<>();

        for (Jugador jugador : jugadores) {
            List<Apuesta> apuestasJugador = obtenerApuestas(jugador); // Obtienes la lista de apuestas del jugador
            if (apuestasJugador != null) {
                for (Apuesta apuesta : apuestasJugador) {
                if (apuesta != null && apuesta.ganado(resultadoRuleta)) {
